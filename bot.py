...
def format_graph_text(graph):
    ...
    for t in sorted_times:
        val = str(times[t])
        if val == "1":
            status_icon, status_text = "🔴", "Немає світла"
        elif val == "10":
            status_icon, status_text = "⚪", "Можливе відключення"
        elif val == "0":
            status_icon, status_text = "🟢", "Світло є"
        else:
            status_icon, status_text = "❓", "Невідомо"
...
async def monitoring_job(context: ContextTypes.DEFAULT_TYPE):
    logger.info("Фонова перевірка оновлень графіку...")
    for delta in [0, 1]:
            ...
            if times_data and has_graph_changed(date_text, times_data):
                ...
                for user_id in users:
                    try:
                        ...
                        logger.warning(f"Не надіслано користувачу {user_id}: {e}")
